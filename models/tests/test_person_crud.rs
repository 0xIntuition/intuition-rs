...
mod tests {
    ...
    async fn test_person_upsert_and_find() -> Result<(), ModelError> {
        ...
        let stored_person = initial_person.upsert(TEST_SCHEMA, &pool).await?;
        ...
        let stored_updated_person = updated_person.upsert(TEST_SCHEMA, &pool).await?;
        assert_eq!(
        ...
        // Test find_by_id
        let found_person = Person::find_by_id(initial_person.id.clone(), TEST_SCHEMA, &pool)
            .await?
