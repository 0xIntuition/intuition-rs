-- Fix pnl_pct (ROI) denominator to use total capital deployed
--
-- BUG:
--   The pnl_pct formula used a CASE expression that picked equity_at_start
--   as the denominator whenever it was > 0, completely ignoring in-period
--   deposits. This produced absurd ROI values for accounts that:
--     1. Had a small pre-existing position at period start (e.g. 5 ETH)
--     2. Made large deposits during the period (e.g. 1,106 ETH)
--     3. Earned profit primarily from the in-period deposits
--
--   Example: account 0x83D3...CcA5 showed 27,299% ROI because the formula
--   divided 1,370 ETH profit by 5 ETH starting equity, ignoring the 1,106 ETH
--   deposited during the period.
--
-- FIX:
--   Replace the CASE expression with (equity_at_start + period_deposits_raw),
--   which represents total capital deployed during the period. This correctly
--   accounts for both pre-existing equity and in-period deposits.
--
--   Edge cases:
--     - equity_at_start=0, deposits>0: denominator = deposits (same as old fallback)
--     - equity_at_start>0, deposits=0: denominator = equity_at_start (same as old primary)
--     - both > 0: denominator = sum (the actual fix)
--     - both = 0: NULLIF returns NULL, COALESCE returns 0%
...
BEGIN
  ...
  enriched AS (
    SELECT
      ...
      -- FIX: Use total capital deployed (equity_at_start + period_deposits)
      -- as the denominator instead of the old CASE expression that ignored
      -- in-period deposits whenever equity_at_start > 0.
      COALESCE(
