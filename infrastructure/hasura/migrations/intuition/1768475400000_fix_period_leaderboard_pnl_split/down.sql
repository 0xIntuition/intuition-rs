-- Down migration: restore original period leaderboard functions with old realized/unrealized PnL logic
-- This reverts to the buggy behavior where:
--   realized_pnl = redemptions - deposits (negated cash flow, not actual realized PnL)
--   unrealized_pnl = equity_end - equity_start - (deposits - redemptions) (algebraically == total_pnl)
