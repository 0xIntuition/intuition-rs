-- Migration: Add raw-table fallback for start-of-period price lookups
--
-- Bug: share_price_change_stats_hourly cagg is bucketed on updated_at (DB write time),
-- which only has data from 2026-02-26 onward. Any period starting before that date
-- gets NULL start prices, causing equity_at_start = 0 and massively inflated PnL.
--
-- Fix: After the cagg-based _tmp_prices_at_start lookup, insert missing prices from
-- the raw share_price_change table using block_timestamp (on-chain time), which has
-- full historical data. Uses the existing idx_share_price_change_term_curve_block_time
-- index for efficient lookups. The NOT EXISTS filter ensures this is a no-op when
-- the cagg already has data for the requested date.
...
BEGIN
  ...
  -- Day-level boundaries for position_change_daily (Step 1)
  v_bucket_start := date_trunc('day', p_start_date);
  v_bucket_end   := date_trunc('day', p_end_date);

  -- Hour-level boundaries for position_cumulative_hourly (Step 2)
  ...
  ---------------------------------------------------------------------------
  -- Step 1: Active accounts (day-level boundaries — unchanged)
  ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------
  -- Step 2: Position data from cumulative snapshots (hour-level boundaries)
  ---------------------------------------------------------------------------
  ...
  ---------------------------------------------------------------------------
  -- Step 3: Price lookups using the cagg + raw-table fallbacks
  ---------------------------------------------------------------------------

  -- prices_at_start: latest share price at or before p_start_date (from cagg)
  ...
  -- Fallback: for vaults with shares at start but no cagg entry before p_start_date,
  -- query the raw share_price_change table using block_timestamp (on-chain time).
  -- Uses idx_share_price_change_term_curve_block_time for efficient index seeks.
  -- Self-optimizing: NOT EXISTS eliminates all rows when the cagg has full coverage.
  INSERT INTO _tmp_prices_at_start (term_id, curve_id, share_price)
  ...
  WHERE spc.block_timestamp <= v_start_timestamp
    AND NOT EXISTS (
      SELECT 1 FROM _tmp_prices_at_start ps
      WHERE ps.term_id = spc.term_id AND ps.curve_id = spc.curve_id
    )
  ORDER BY spc.term_id, spc.curve_id, spc.block_timestamp DESC, spc.log_index DESC;

  -- prices_at_end: latest share price at or before p_end_date (from cagg)
...
COMMENT ON FUNCTION get_pnl_leaderboard_period IS
  ...
  'Step 2 uses position_cumulative_hourly point-in-time lookups with hour-level boundaries. '
  'Step 3 uses share_price_change_stats_hourly cagg for price lookups with '
  'raw-table fallback for start prices when the cagg has no historical data, '
  'and raw-table patch for the current incomplete hour on end prices. '
