-- Fix period functions: switch from share-ratio to output-ratio for realized PnL
--
-- shares_delta_period is a NET daily figure. When deposit and redemption happen
-- on the same day, net is positive, so shares_removed_in_period = 0. This caused
-- realized_pnl = total_redemptions (zero cost deducted) and denominator_closed = 0.
--
-- Fix: use output-ratio method (period_redemptions / (period_redemptions + equity_at_end))
-- which uses gross asset flows instead of net share deltas.
