-- Rewrite get_pnl_leaderboard_period Step 2 to use position_cumulative_hourly
-- instead of the LATERAL full-history scan over position_change_daily.
--
-- The old Step 2 performed a CROSS JOIN LATERAL that, for each active account,
-- scanned every row in position_change_daily up to v_bucket_end.  On a large
-- dataset this is O(accounts * rows_per_account) and was the dominant cost.
--
-- The new Step 2 replaces that scan with two point-in-time lookups per
-- (account, term, curve):
--   snap_end   – latest cumulative row with bucket <= v_bucket_end
--   snap_start – latest cumulative row with bucket <  v_bucket_start (strictly less)
--
-- All downstream CTEs (prices_at_start, prices_at_end, position_period_metrics,
-- position_pnl, filtered_pnl, account_metrics, enriched, ranked, final SELECT)
-- are IDENTICAL to migration 1771526422000.  The _tmp_position_data temp table
-- exposes the same column names and types so nothing downstream changes.
--
-- Function output (columns, values, ordering) is identical to the previous version.
...
BEGIN
  ...
  -- Bump work_mem to avoid temp I/O spills in hash aggregation
  SET LOCAL work_mem = '64MB';

  -- Step 1: Materialize active accounts so planner sees real cardinality.
  -- Protocol accounts (ProtocolVault, AtomWallet) are excluded here when
  -- p_exclude_protocol_accounts is TRUE, avoiding their propagation through
  -- the entire pipeline.  The WHERE clause in the enriched CTE remains as a
  -- safety net but will be a no-op for the common case.
  ...
  -- Step 2: Position data from cumulative snapshots (replaces LATERAL full-history scan).
  --
  -- For each active account we grab two snapshots from position_cumulative_hourly:
  --   snap_end   : latest bucket <= v_bucket_end  (gives cumulative totals at period end)
  --   snap_start : latest bucket <  v_bucket_start (gives cumulative totals before period)
  --
  -- Subtracting snap_start from snap_end yields the in-period deltas.
  -- Positions that have only pre-period history still appear because snap_end
  -- uses bucket <= v_bucket_end (not >= v_bucket_start), so any row ever
  -- written for the account will be found as long as it predates v_bucket_end.
  ...
  SELECT
    ...
    -- had_activity: true if any assets moved in or out during the period.
    -- Equivalent to the previous BOOL_OR(bucket >= v_bucket_start AND bucket <= v_bucket_end):
    -- every row in position_cumulative_hourly represents a bucket with deposits or
    -- redemptions, so a change in cumulative_assets_in or cumulative_assets_out is
    -- exactly "there was a row in the period".
  ...
  FROM (
    -- For each active account, get the latest cumulative snapshot at or before
    -- period end for every (term_id, curve_id) they have ever had activity in.
    -- Using bucket <= v_bucket_end (not >= v_bucket_start) ensures positions
    -- with only pre-period history are included in _tmp_position_data.
  ...
  LEFT JOIN LATERAL (
    -- Latest cumulative snapshot STRICTLY BEFORE period start.
    -- Strictly less than matches the previous FILTER (WHERE pc.bucket < v_bucket_start).
    SELECT
      cumulative_shares,
  ...
  -- Step 3: Main query using materialized temp tables
  RETURN QUERY
  WITH
  -- Derive vault list from position_data (no redundant second scan)
  prices_at_start AS (
    SELECT DISTINCT ON (spc.term_id, spc.curve_id)
      spc.term_id, spc.curve_id, spc.share_price
    FROM share_price_change spc
    WHERE EXISTS (
      SELECT 1 FROM _tmp_position_data pd
      WHERE pd.term_id = spc.term_id AND pd.curve_id = spc.curve_id
        AND COALESCE(pd.shares_at_start, 0) > 0
    )
    AND spc.block_timestamp <= v_start_timestamp
  ...
  enriched AS (
    ...
    -- Safety net: Step 1 already excludes protocol accounts when
    -- p_exclude_protocol_accounts is TRUE, so this filter is a no-op in the
    -- common case but preserves correctness if called with pre-filtered data.
...
COMMENT ON FUNCTION get_pnl_leaderboard_period IS
  ...
  'Perf: protocol accounts excluded at Step 1 (pre-filter); Step 2 uses '
  'position_cumulative_hourly point-in-time lookups instead of full-history LATERAL scan.';
