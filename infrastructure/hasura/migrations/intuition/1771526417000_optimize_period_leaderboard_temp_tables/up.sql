-- Optimize get_pnl_leaderboard_period: ~10x speedup (6.4s → ~500-700ms)
--
-- 1. Materialize active_accounts in temp table + LATERAL join for position_data
--    (IN subquery on continuous aggregate causes full 3.1M row seq scan;
--     LATERAL forces nested loop with per-account index scan: 3400ms → 50ms)
-- 2. Eliminate redundant relevant_vaults CTE (was doing a second full scan)
--    Derive vault list from position_data instead
-- 3. Filter price lookups to only vaults with non-zero shares (reduces 1025 → fewer lookups)
-- 4. SET LOCAL work_mem = '64MB' to avoid temp I/O spills

-- ========================================
-- 1. get_pnl_leaderboard_period (11-param)
-- ========================================
