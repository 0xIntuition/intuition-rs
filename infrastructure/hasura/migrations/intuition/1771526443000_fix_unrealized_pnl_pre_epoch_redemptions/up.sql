-- Fix unrealized PnL inflated by already-redeemed pre-epoch capital.
--
-- Problem: unrealized_pnl was computed as total_pnl - realized_pnl. When a user
-- redeems a pre-epoch position (period_deposits = 0), realized_pnl is correctly
-- forced to 0 (epoch-only rule), but the redemption profit gets dumped into
-- unrealized_pnl. This inflates "unrealized" with already-cashed-out value.
--
-- Fix: For positions with no in-epoch deposits but redemptions during the epoch,
-- compute unrealized as the change in remaining equity only (equity_at_end minus
-- the value of remaining shares at epoch-start prices), instead of the residual
-- total_pnl - realized_pnl. This ensures unrealized PnL only reflects value that
-- hasn't been cashed out yet.
--
-- DEPENDS ON:
--   1771526440000_fifo_chronological_realized_pnl
--
-- SCOPE:
--   Modified: get_pnl_leaderboard_period, get_vault_leaderboard_period
--   Changed:  equity_of_remaining_at_start column in position_period_metrics,
--             unrealized_pnl_raw aggregation in account_metrics
--   Unchanged: realized_pnl, denominators, FIFO, bonding curve equity, position_data
...
--    Changes vs 1771526440000:
--    - Added equity_of_remaining_at_start to position_period_metrics
--    - Fixed unrealized_pnl_raw in account_metrics for pre-epoch redemptions
-- ============================================================================
...
BEGIN
  ...
  position_period_metrics AS (
    SELECT
      ...
      CASE
        WHEN COALESCE(pss.curve_id, pse.curve_id, pa.curve_id) = 2
             AND COALESCE(pse.shares_at_end, 0) > 0
             AND COALESCE(pss.shares_at_start, 0) > 0 THEN
          TRUNC(
            (
              TRUNC(
              ...
              TRUNC(
                (
                  (COALESCE(pbs.total_shares, pe_earliest.total_shares, 0) + cc."offset" - GREATEST(COALESCE(pse.shares_at_end, 0), 0))
                  * (COALESCE(pbs.total_shares, pe_earliest.total_shares, 0) + cc."offset" - GREATEST(COALESCE(pse.shares_at_end, 0), 0))
                  + 999999999999999999::NUMERIC
                )
                / 1000000000000000000::NUMERIC
        ...
        WHEN COALESCE(pse.shares_at_end, 0) > 0
             AND COALESCE(pss.shares_at_start, 0) > 0 THEN
          TRUNC(
            GREATEST(COALESCE(pse.shares_at_end, 0), 0)
            * COALESCE(pbs.share_price, pe_earliest.share_price)
            / 1000000000000000000::NUMERIC
          )
        ELSE 0
      END AS equity_of_remaining_at_start,
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(
        CASE
          WHEN pf.period_deposits <= 0 AND pf.period_redemptions > 0 THEN
            GREATEST(pf.equity_at_end, 0) - pf.equity_of_remaining_at_start
          ELSE
            pf.period_total_pnl - pf.realized_pnl
        END
      )::NUMERIC AS unrealized_pnl_raw,
