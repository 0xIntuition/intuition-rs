-- Optimize get_pnl_leaderboard_period performance
--
-- Problem: The prices_before_start, prices_earliest, and prices_at_end CTEs
-- scan the ENTIRE share_price_change table (300K+ vaults) via DISTINCT ON,
-- even though only a small subset of vaults are relevant to active accounts.
-- This causes ~14s query times due to 3.5M+ buffer hits.
--
-- Fix: Add a relevant_vaults CTE that pre-computes which (term_id, curve_id)
-- pairs are actually needed, then filter all price lookups to only those vaults.
