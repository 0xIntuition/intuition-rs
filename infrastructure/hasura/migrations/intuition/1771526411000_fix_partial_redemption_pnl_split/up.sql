-- Fix partial redemption PnL split in all 4 leaderboard functions
--
-- Previously, the realized/unrealized PnL split was binary per-position:
-- if shares_at_end <= 0 all PnL was "realized", otherwise all "unrealized".
-- This meant partial redemptions never showed realized gains.
--
-- Now uses a cost-basis proportional split:
--   All-time:  output-ratio method using total_redemptions / (total_redemptions + equity)
--   Period:    share-ratio method using shares_removed / (shares_at_end + shares_removed)
--
...
-- ========================================
-- 3. get_pnl_leaderboard_period (11-param)
-- ========================================
...
BEGIN
  ...
  position_data AS (
    SELECT
      ...
      SUM(GREATEST(-pc.shares_delta_period, 0)) FILTER (WHERE pc.bucket >= v_bucket_start)::NUMERIC
        AS shares_removed_in_period
    FROM position_change_daily pc
  ...
  position_period_metrics AS (
    SELECT
      ...
      COALESCE(pd.shares_removed_in_period, 0)   AS shares_removed_in_period,
      TRUNC(
  ...
  position_pnl AS (
    SELECT
      ...
      -- Proportional realized PnL using cost-basis share-ratio method
      CASE
        ...
        ELSE (ppm.period_redemptions - TRUNC(
          (ppm.equity_at_start + ppm.period_deposits) * ppm.shares_removed_in_period
          / NULLIF(ppm.shares_at_end + ppm.shares_removed_in_period, 0)
        ))::NUMERIC
      END AS realized_pnl
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(CASE
        ...
        ELSE TRUNC((fp.equity_at_start + fp.period_deposits) * fp.shares_removed_in_period
             / NULLIF(fp.shares_at_end + fp.shares_removed_in_period, 0))
      END)::NUMERIC                                              AS denominator_closed,
      SUM(CASE
        WHEN fp.shares_at_end <= 0 THEN 0
        WHEN fp.period_redemptions <= 0 THEN fp.equity_at_start + fp.period_deposits
        ELSE (fp.equity_at_start + fp.period_deposits) - TRUNC((fp.equity_at_start + fp.period_deposits) * fp.shares_removed_in_period
             / NULLIF(fp.shares_at_end + fp.shares_removed_in_period, 0))
      END)::NUMERIC                                              AS denominator_open
...
COMMENT ON FUNCTION get_pnl_leaderboard_period IS
  ...
  'gaming with tiny stakes. Uses cost-basis proportional split for partial redemptions.';
...
BEGIN
  ...
  period_activity AS (
    SELECT
      ...
      SUM(GREATEST(-pc.shares_delta_period, 0))::NUMERIC AS shares_removed_in_period
    FROM position_change_daily pc
    WHERE pc.bucket >= v_bucket_start
      AND pc.bucket <= v_bucket_end
      AND pc.term_id = p_term_id
  ...
  position_period_metrics AS (
    SELECT
      ...
      COALESCE(pa.shares_removed_in_period, 0) AS shares_removed_in_period,
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(CASE
        ...
        ELSE TRUNC((pf.equity_at_start + pf.period_deposits) * pf.shares_removed_in_period
             / NULLIF(pf.shares_at_end + pf.shares_removed_in_period, 0))
      END)::NUMERIC AS denominator_closed,
      SUM(CASE
        WHEN pf.shares_at_end <= 0 THEN 0
        WHEN pf.period_redemptions <= 0 THEN pf.equity_at_start + pf.period_deposits
        ELSE (pf.equity_at_start + pf.period_deposits) - TRUNC((pf.equity_at_start + pf.period_deposits) * pf.shares_removed_in_period
             / NULLIF(pf.shares_at_end + pf.shares_removed_in_period, 0))
      END)::NUMERIC AS denominator_open
