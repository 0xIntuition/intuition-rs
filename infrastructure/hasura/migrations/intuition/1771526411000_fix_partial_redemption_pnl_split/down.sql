-- Revert: restore binary per-position realized/unrealized PnL split
-- Restores functions 1, 2, 4 from 1771526409000 and function 3 from 1771526410000

-- ========================================
-- 1. get_pnl_leaderboard (all-time)
--    Restored from 1771526409000
-- ========================================
...
BEGIN
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(fp.period_total_pnl) FILTER (WHERE fp.shares_at_end <= 0)::NUMERIC
                                                                 AS realized_pnl_raw,
      SUM(fp.period_total_pnl) FILTER (WHERE fp.shares_at_end > 0)::NUMERIC
      ...
      SUM(fp.equity_at_start + fp.period_deposits) FILTER (WHERE fp.shares_at_end <= 0)::NUMERIC
