-- Add p_min_deposit parameter to get_pnl_leaderboard_period
--
-- Filters out positions where cumulative all-time deposits are below
-- a configurable threshold (e.g., 5 TRUST). This prevents users from
-- gaming the leaderboard with tiny stakes across many positions.
--
-- The parameter is in ETH/TRUST units (not wei). Internally multiplied by 1e18.
-- Default 0 = no filtering (backward compatible).

-- Must DROP first because CREATE OR REPLACE cannot change parameter list
DROP FUNCTION IF EXISTS get_pnl_leaderboard_period(
  TIMESTAMPTZ, TIMESTAMPTZ, INTEGER, INTEGER, TEXT, TEXT, BOOLEAN, INTEGER, NUMERIC, TEXT
);

CREATE FUNCTION get_pnl_leaderboard_period(
