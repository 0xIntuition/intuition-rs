-- Fix medium-severity bugs in leaderboard functions:
-- Bug 1: Float 1e18 precision loss → use 1000000000000000000::NUMERIC + TRUNC()
-- Bug 2: active_position_count can exceed total_position_count (period functions)
-- Bug 3: COALESCE(share_price, 0) hides missing price data (period functions)
-- Bonus: shares = 0 → shares <= 0 for realized PnL filter (all-time functions)
