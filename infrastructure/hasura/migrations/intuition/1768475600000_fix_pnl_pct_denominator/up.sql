-- Fix pnl_pct denominator bug:
-- Problem: pnl_pct used (deposits - redemptions) as denominator, which:
--   1. Always produces -100% for fully-exited losing positions
--   2. Goes negative for profitable accounts with large redemptions, flipping the sign
--   3. Hides gains entirely when denominator is negative (ELSE 0 branch in view)
-- Fix: Use total_deposits as denominator = standard ROI (return / total capital invested)
-- Scope: position_with_value view + all 4 leaderboard functions

-- ========================================
-- 0. FIX position_with_value VIEW
-- ========================================
...
BEGIN
  ...
  current_positions AS (
    SELECT
      ...
      -- Bug 1 fix: use NUMERIC division + TRUNC for Solidity-matching integer math
  ...
  account_metrics AS (
    SELECT
      ...
      -- Bonus fix: shares <= 0 to catch negative-share data anomalies
      ...
      CASE
        ...
        WHEN p.curve_id = 2 THEN
          (
            SELECT TRUNC(
              (
                -- sSquared (rounds down via TRUNC)
                ...
                -
                -- sNextSquared (rounds up: (x + 1e18 - 1) / 1e18 truncated)
              ...
              * 50000000000000000::NUMERIC / 1000000000000000000::NUMERIC  -- HALF_SLOPE
            )
          )
        ELSE 0::NUMERIC
...
BEGIN
  ...
  -- Bug 3 fix: preferred prices at period START (latest price at or before start)
  ...
  -- Bug 3 fix: fallback prices for vaults with no price before start (earliest known price)
  prices_earliest AS (
    SELECT DISTINCT ON (term_id, curve_id)
      term_id, curve_id, share_price, total_assets, total_shares
    FROM share_price_change
  ...
  position_period_metrics AS (
    SELECT
      ...
      -- Historical prices (Bug 3 fix: fallback chain instead of COALESCE(price, 0))
      COALESCE(pbs.share_price, pe_earliest.share_price) AS price_at_start,
      COALESCE(pe.share_price, 0) AS price_at_end,
      -- Bug 1 fix: use NUMERIC division + TRUNC for equity calculations
    ...
    -- Bug 3 fix: join preferred prices first, then fallback
  ...
  position_pnl AS (
    SELECT
      ppm.*,
      -- Total period PnL = final equity - initial equity + cash withdrawn - cash deposited
  ...
  account_metrics AS (
    SELECT
      pp.account_id,
      -- Bug 2 fix: total = positions with activity OR still held at end
      ...
      -- Realized PnL = total PnL from positions fully exited by period end (shares <= 0)
      SUM(pp.period_total_pnl) FILTER (WHERE pp.shares_at_end <= 0)::NUMERIC AS realized_pnl_raw,
      -- Unrealized PnL = total PnL from positions still held at period end (shares > 0)
    ...
    -- HAVING uses had_activity to only require positions with actual trading activity
