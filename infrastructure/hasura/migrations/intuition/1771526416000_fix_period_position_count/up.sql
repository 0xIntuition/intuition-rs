-- Fix: period position counts should only include positions with activity in the period
--
-- period_position_count previously used: FILTER (WHERE had_activity OR shares_at_end > 0)
-- winning_positions/losing_positions had no activity filter at all.
-- This inflated counts by including dormant positions with open shares.
--
-- Fix: All three metrics now use FILTER (WHERE had_activity)
...
BEGIN
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(pf.is_winning) FILTER (WHERE pf.had_activity) AS winning_positions,
      SUM(pf.is_losing) FILTER (WHERE pf.had_activity) AS losing_positions,
      ...
      SUM(CASE
        ...
        ELSE TRUNC((pf.equity_at_start + pf.period_deposits) * pf.shares_redeemed_in_period
      ...
      SUM(CASE
        ...
        ELSE TRUNC((pf.equity_at_start + pf.period_deposits) * pf.shares_at_end
             / NULLIF(pf.shares_at_start + pf.shares_acquired_in_period, 0))
      END)::NUMERIC AS denominator_open
