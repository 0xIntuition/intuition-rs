-- Fix daily bucket alignment for period leaderboard functions
--
-- Problem: position_change_daily uses time_bucket('1 day', ...) which truncates
-- timestamps to midnight (00:00 UTC). When callers pass mid-day timestamps like
-- '2026-02-24 12:00:00', the filter `bucket >= p_start_date` becomes
-- '2026-02-24 00:00:00 >= 2026-02-24 12:00:00' which is FALSE, dropping the
-- entire day's data. This causes empty results for valid date ranges.
--
-- Fix: Truncate date parameters to day boundaries when comparing against
-- position_change_daily buckets. Keep precise timestamps for share_price_change
-- lookups which use Unix epoch seconds.
--
-- Affected functions:
--   - get_pnl_leaderboard_period
--   - get_vault_leaderboard_period
--   - get_pnl_leaderboard (active_accounts and period_pnl_change CTEs)

-- ========================================
-- FIX get_pnl_leaderboard_period
-- ========================================
...
BEGIN
  ...
  -- Pre-compute which vaults are relevant (positions held by active accounts)
  -- This limits price lookups from 300K+ vaults to only the ones that matter
  relevant_vaults AS (
  ...
  -- Historical share prices at period START - only for relevant vaults
  prices_before_start AS (
    SELECT DISTINCT ON (spc.term_id, spc.curve_id)
      spc.term_id, spc.curve_id, spc.share_price, spc.total_assets, spc.total_shares
    FROM share_price_change spc
    INNER JOIN relevant_vaults rv ON spc.term_id = rv.term_id AND spc.curve_id = rv.curve_id
    WHERE spc.block_timestamp <= v_start_timestamp
    ORDER BY spc.term_id, spc.curve_id, spc.block_timestamp DESC, spc.log_index DESC
  ),

  -- Earliest known price per vault (fallback) - only for relevant vaults
  prices_earliest AS (
    SELECT DISTINCT ON (spc.term_id, spc.curve_id)
      spc.term_id, spc.curve_id, spc.share_price, spc.total_assets, spc.total_shares
    FROM share_price_change spc
    INNER JOIN relevant_vaults rv ON spc.term_id = rv.term_id AND spc.curve_id = rv.curve_id
    ORDER BY spc.term_id, spc.curve_id, spc.block_timestamp ASC, spc.log_index ASC
  ),

  -- Historical share prices at period END - only for relevant vaults
  prices_at_end AS (
    SELECT DISTINCT ON (spc.term_id, spc.curve_id)
      spc.term_id, spc.curve_id, spc.share_price, spc.total_assets, spc.total_shares
    FROM share_price_change spc
  ...
  -- Position state at period START - only for active accounts
  ...
  -- Position state at period END - only for active accounts
  ...
  -- Period activity (deposits and redemptions DURING the period)
  period_activity AS (
    SELECT
      pc.account_id, pc.term_id, pc.curve_id,
  ...
  -- Combine position data with historical prices
...
-- ========================================
-- FIX get_vault_leaderboard_period
-- ========================================
