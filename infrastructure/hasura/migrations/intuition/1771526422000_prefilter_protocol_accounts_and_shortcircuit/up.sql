-- Performance optimization: pre-filter protocol accounts in Step 1 (avoids processing them
-- through the entire pipeline) and short-circuit the pre-period scan when there is no
-- historical data before the requested period window.
...
BEGIN
  ...
  -- Check if any data exists before the period start.
  -- When the dataset starts at or after v_bucket_start there is no pre-period
  -- history, so we can add a lower-bound on bucket in the LATERAL scan and
  -- avoid reading the full historical table for every account.
  SELECT EXISTS (
    SELECT 1 FROM position_change_daily
    WHERE bucket < v_bucket_start
    LIMIT 1
  ) INTO v_has_pre_period_data;

  -- Step 2: Materialize position_data using LATERAL join to force index usage per account
  -- (IN subquery causes planner to choose hash semi join + full 3.1M row seq scan;
  --  LATERAL forces nested loop with index scan per account: 3400ms → 50ms)
  ...
  CROSS JOIN LATERAL (
    ...
    FROM position_change_daily pc
    WHERE pc.account_id = aa.account_id
      AND pc.bucket <= v_bucket_end
      AND (v_has_pre_period_data OR pc.bucket >= v_bucket_start)
      AND (p_term_id IS NULL OR pc.term_id = p_term_id)
...
COMMENT ON FUNCTION get_pnl_leaderboard_period IS
  ...
  'Perf: protocol accounts excluded at Step 1 (pre-filter); pre-period bucket scan '
  'short-circuited when no historical data predates the requested window.';
