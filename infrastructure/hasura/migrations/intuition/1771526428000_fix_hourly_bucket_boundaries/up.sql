-- Migration: Fix hourly bucket boundary mismatch in get_pnl_leaderboard_period
--
-- Bug: v_bucket_start/v_bucket_end used date_trunc('day') but were applied against
-- position_cumulative_hourly (hourly buckets). This shifted the snap_start boundary
-- by up to 23 hours, causing pre-period events to leak into the period calculation.
--
-- Example: period starting at Feb 24 12:00 PM would truncate to Feb 24 00:00,
-- missing any cumulative hourly bucket between 00:00 and 12:00. A redemption at
-- 11:57 AM (bucket 11:00) would be incorrectly counted as period activity.
--
-- Fix: Add v_hour_start/v_hour_end variables truncated to hour granularity,
-- used exclusively in Step 2 (position_cumulative_hourly lookups).
-- Step 1 (position_change_daily) retains day-level boundaries unchanged.
...
BEGIN
  ...
  ---------------------------------------------------------------------------
  -- Step 2: Position data from cumulative snapshots (FIXED: hour-level boundaries)
  ---------------------------------------------------------------------------
...
COMMENT ON FUNCTION get_pnl_leaderboard_period IS
  ...
  '(v_hour_start/v_hour_end) instead of day-level truncation to prevent pre-period events '
  'from leaking into period calculations. '
