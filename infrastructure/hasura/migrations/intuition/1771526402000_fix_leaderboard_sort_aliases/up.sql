-- Fix sort aliases for leaderboard functions
--
-- Problem: The CASE p_sort_by statement only matches exact values like 'pnl_pct',
-- but users/frontend may send 'roi' or 'pnl'. Unrecognized values fall through
-- to the ELSE clause which sorts by total_pnl_raw, causing unexpected ordering.
--
-- Fix: Use CASE WHEN ... IN (...) to support common aliases:
--   'total_pnl' or 'pnl' -> sort by absolute PnL
--   'pnl_pct' or 'roi' -> sort by ROI percentage
--   'total_volume' or 'volume' -> sort by volume
--   'position_count' or 'positions' -> sort by position count

-- Note: get_pnl_leaderboard_period aliases are handled in migration
-- 1771526401000_optimize_period_leaderboard_performance
