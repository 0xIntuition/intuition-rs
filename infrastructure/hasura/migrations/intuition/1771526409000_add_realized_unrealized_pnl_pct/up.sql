-- Add realized_pnl_pct and unrealized_pnl_pct to pnl_leaderboard_entry
--
-- These columns provide ROI percentages broken down by closed vs open positions:
--   realized_pnl_pct   = realized_pnl * 100 / denominator_for_closed_positions
--   unrealized_pnl_pct = unrealized_pnl * 100 / denominator_for_open_positions
--
-- All-time functions use total_deposits as denominator.
-- Period functions use (equity_at_start + period_deposits) as denominator.
--
-- Bonus: get_vault_leaderboard_period pnl_pct denominator is fixed to match
-- the corrected formula from get_pnl_leaderboard_period (migration 1771526405000).

-- ========================================
-- 0. RECREATE pnl_leaderboard_entry TABLE
-- ========================================
...
BEGIN
  ...
  -- Truncate to day boundaries for position_change_daily bucket comparisons
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(cp.total_deposits_raw) FILTER (WHERE cp.shares <= 0)::NUMERIC AS deposits_on_closed_positions,
      SUM(cp.total_deposits_raw) FILTER (WHERE cp.shares > 0)::NUMERIC AS deposits_on_open_positions
  ...
  enriched AS (
    SELECT
      ...
      COALESCE(
        (COALESCE(am.realized_pnl_raw, 0) * 100.0 / NULLIF(am.deposits_on_closed_positions, 0))::NUMERIC(20, 4),
        0::NUMERIC(20, 4)
      ) AS realized_pnl_pct,
      COALESCE(
        (COALESCE(am.unrealized_pnl_raw, 0) * 100.0 / NULLIF(am.deposits_on_open_positions, 0))::NUMERIC(20, 4),
        0::NUMERIC(20, 4)
...
BEGIN
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(vp.total_deposits_raw) FILTER (WHERE vp.shares <= 0)::NUMERIC AS deposits_on_closed_positions,
      SUM(vp.total_deposits_raw) FILTER (WHERE vp.shares > 0)::NUMERIC AS deposits_on_open_positions
...
BEGIN
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(pp.equity_at_start + pp.period_deposits) FILTER (WHERE pp.shares_at_end <= 0)::NUMERIC
                                                                 AS denominator_closed,
      SUM(pp.equity_at_start + pp.period_deposits) FILTER (WHERE pp.shares_at_end > 0)::NUMERIC
                                                                 AS denominator_open
...
COMMENT ON FUNCTION get_pnl_leaderboard_period IS
  'Period-specific PnL leaderboard. Uses total capital deployed (equity_at_start + period_deposits) '
  'as the ROI denominator to prevent inflated percentages from small starting positions with large '
  'in-period deposits. Includes realized_pnl_pct and unrealized_pnl_pct.';

-- ========================================
-- 4. get_vault_leaderboard_period
--    Base: 1771526403000
--    Fix: pnl_pct denominator now uses
--         (equity_at_start + period_deposits)
-- ========================================
...
BEGIN
  ...
  -- Precise timestamps for share_price_change lookups
  v_start_timestamp := EXTRACT(EPOCH FROM p_start_date)::BIGINT;
  v_end_timestamp := EXTRACT(EPOCH FROM p_end_date)::BIGINT;
  ...
  account_metrics AS (
    SELECT
      ...
      SUM(pf.equity_at_start + pf.period_deposits) FILTER (WHERE pf.shares_at_end <= 0)::NUMERIC AS denominator_closed,
      SUM(pf.equity_at_start + pf.period_deposits) FILTER (WHERE pf.shares_at_end > 0)::NUMERIC AS denominator_open
  ...
  enriched AS (
    SELECT
      ...
      COALESCE(am.realized_pnl_raw, 0) AS realized_pnl_raw,
      COALESCE(am.unrealized_pnl_raw, 0) AS unrealized_pnl_raw,
      -- FIX: Use (equity_at_start + period_deposits) instead of CASE expression
      COALESCE(
        (am.total_pnl_raw * 100.0 / NULLIF(
          am.equity_at_start + am.period_deposits_raw
        , 0))::NUMERIC(20, 4),
