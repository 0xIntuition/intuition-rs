...
where
    T: SharePriceChangedEvent + Debug + Sync + Send,
{
    async fn process_event(
        ...
        info!("Handling SharePriceChanged event: {self:#?}",);
        ...
        match SharePriceChange::fetch_share_price_from_internal(
            &SharePriceChangeInternal::builder()
                .term_id(FixedBytesWrapper::from(self.0.term_id()?))
                .vault_type(self.0.vault_type()?)
                .curve_id(self.0.curve_id()?)
                .share_price(self.0.new_share_price()?)
                .total_assets(self.0.total_assets()?)
                .total_shares(self.0.total_shares()?)
        ...
        self.0
            .update_vault_from_share_price_changed_events(decoded_consumer_context, event)
            .await?;
