...
use crate::services::{fetch_pnl_leaderboard_period, fetch_pnl_leaderboard_period_min_threshold};
use crate::state::AppState;
use crate::types::{PnlLeaderboardPeriodMinThresholdQueryParams, PnlLeaderboardPeriodQueryParams};
...
/// Handler for PnL leaderboard period endpoint with min deposit threshold
/// GET /api/v1/leaderboard/pnl/period/min-threshold
#[utoipa::path(
    get,
    path = "/api/v1/leaderboard/pnl/period/min-threshold",
    params(
        ("start" = String, Query, description = "Range start timestamp (unix seconds, unix milliseconds, or RFC3339)"),
        ("end" = String, Query, description = "Range end timestamp (unix seconds, unix milliseconds, or RFC3339)"),
        ("limit" = Option<i32>, Query, description = "Max results to return (1-10000, default 100)"),
        ...
        ("min_volume" = Option<f64>, Query, description = "Minimum volume filter in ETH (default 0)"),
        ("term_id" = Option<String>, Query, description = "Optional term_id filter"),
        ("min_deposit" = Option<f64>, Query, description = "Minimum cumulative deposit threshold in ETH/TRUST (default 0)"),
    ),
    responses(
        (status = 200, description = "PnL leaderboard for the requested period with min deposit filtering", body = Vec<PnlLeaderboardEntrySchema>, content_type = "application/json"),
        (status = 400, description = "Invalid parameters"),
        (status = 404, description = "No data found"),
        (status = 500, description = "Internal server error"),
    ),
...
pub async fn get_pnl_leaderboard_period_min_threshold(
    State(state): State<AppState>,
    Query(params): Query<PnlLeaderboardPeriodMinThresholdQueryParams>,
) -> Result<Response, ApiError> {
    // Parse and validate timestamps
    let start = parse_timestamp(&params.start)
        .ok_or_else(|| ApiError::InvalidStartTimestamp(params.start.clone()))?;
    ...
    let min_volume = BigDecimal::try_from(min_volume_f64)
        .unwrap_or_else(|_| BigDecimal::from(0));
    let term_id = params.term_id.as_deref();
    let min_deposit_f64 = params.min_deposit.unwrap_or(0.0).max(0.0);
    let min_deposit = BigDecimal::try_from(min_deposit_f64)
        .unwrap_or_else(|_| BigDecimal::from(0));
    ...
    // Build deterministic cache key (includes min_deposit for separate caching)
    let cache_key = format!(
        "leaderboard:pnl:period:mt:{}:{}:{}:{}:{}:{}:{}:{}:{:.2}:{}:{:.4}",
        start.timestamp(),
        end.timestamp(),
        limit,
        offset,
    ...
    // Query database
    let rows = fetch_pnl_leaderboard_period_min_threshold(
        &state.pg_pool,
        start,
        end,
        limit,
