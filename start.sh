...
if [ "$INDEXER_SCHEMA" == "local" ]; then
    ...
    while [ "$CONTRACT_ADDRESS" == "0xB4375293a13017BCe71a034bB588786A3D3C7295" ] || [ -z "$CONTRACT_ADDRESS" ]; do
        CONTRACT_ADDRESS=$(docker compose -f docker-compose-shared.yml exec database psql -U postgres -d storage -c "SELECT contract_address FROM histocrawler.app_config WHERE indexer_schema = 'local'" -tA)
        sleep 1
    done
