...
docker compose -p intuition -f docker/docker-compose-shared.yml up database drizzle-studio pgai-installer vectorizer-worker redis redis-setup ipfs safe-content graphql-engine indexer-migrations hasura-migrations prometheus hasura-metric-adapter grafana -d --wait --force-recreate
